...
class Model:
    ...
    def update_weights_for_one_epoch(self, inputs, outputs, learning_rate):
        ...
        shared_coefficient = (-2*learning_rate/n) * (outputs - A_2) * A_2 * (1 - A_2)
        relu_gradient = np.where(A_1 > 0, 1, 0)
        
        self.w1 = self.w1 - (shared_coefficient.T.dot(self.w2).T * relu_gradient).dot(inputs.T)
