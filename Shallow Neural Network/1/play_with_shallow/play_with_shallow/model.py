...
class Model:
    ...
    def update_weights_for_one_epoch(self, inputs, outputs, learning_rate):
        ...
        shared_coefficient = (-2/n) * (outputs - A_2) * ((1 - A_2) * A_2)  # TODO
        relu_gradient = np.where(A_1 > 0, 1, 0)
        
        self.w1 = self.w1 + learning_rate * (((shared_coefficient.T @ self.w2).T * relu_gradient) @ inputs.T) # TODO
        self.w2 = self.w2 + learning_rate * (shared_coefficient @ A_1.T) # TODO
